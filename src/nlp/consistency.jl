...
function consistent_functions(nlps; linear_api = false, rtol = 1.0e-8, exclude = [])
  ...
  @testset "grad and objgrad" begin
    if !(grad in exclude)
      if mapreduce(nlp -> nlp.meta.grad_available, &, nlps)
    ...
    if !(objgrad in exclude)
      if mapreduce(nlp -> nlp.meta.grad_available, &, nlps)
        fs = Any[obj(nlp, x) for nlp in nlps]
        gs = Any[grad(nlp, x) for nlp in nlps]
        gmin = minimum(map(norm, gs))
        for i = 1:N
  ...
  @testset "hess_structure! and hess_coord!" begin
    if !(hess_coord in exclude)
      if mapreduce(nlp -> nlp.meta.hess_available, &, nlps)
  ...
  @testset "hprod and hprod!" begin
    if !(hprod in exclude)
      if mapreduce(nlp -> nlp.meta.hprod_available, &, nlps)
  ...
  @testset "hess and hess_coord" begin
  ...
  if m > 0
    @testset "cons! and objcons!" begin
      if !(cons in exclude)
        fs = Any[obj(nlp, x) for nlp in nlps]
    ...
    @testset "cons! and objcons! -- test_lin" begin
    ...
    @testset "cons! and objcons! -- test_nln" begin
    ...
    @testset "jac_structure! and jac_coord!" begin
      if intersect([jac, jac_coord], exclude) == []
        if mapreduce(nlp -> nlp.meta.jac_available, &, nlps)
      ...
      @testset "jac_structure! and jac_coord -- test_lin!" begin
      ...
      @testset "jac_structure! and jac_coord -- test_nln!" begin
    ...
    @testset "jprod!" begin
      if !(jprod in exclude)
        if mapreduce(nlp -> nlp.meta.jprod_available, &, nlps)
    ...
    @testset "jtprod!" begin
      if !(jtprod in exclude)
        if mapreduce(nlp -> nlp.meta.jtprod_available, &, nlps)
    ...
    @testset "jth_hess" begin
      ...
      if intersect([jth_hess, jth_hess_coord], exclude) == []
        for i = 1:N
          ...
          I, J = hess_structure(nlp)
          @test length(I) == length(J) == length(V) == nlp.meta.nnzh
          @test isapprox(Symmetric(sparse(I, J, V, n, n), :L), Hx, atol=1e-8)
        end
      end
    end

    @testset "jth_hprod" begin
