...
function check_nls_dimensions(nls; exclude = [])
  ...
  if jac_residual ∉ exclude
    @test_throws DimensionError jac_residual(nls, badx)
  end
  if jac_coord_residual ∉ exclude
